...
Effect::Effect( ::lavascript::zone::Zone* zone , WriteEffect* write ):
  write_effect_(write),
  read_list_   (),
  zone_        (zone)
{}
...
void Effect::UpdateWriteEffect( WriteEffect* node ) {
  if(read_list_.empty()) {
...
void Effect::AddReadEffect( ReadEffect* node ) {
...
void Effect::Merge( const Effect& lhs, const Effect& rhs, Effect* output , Graph* graph , ControlFlow* region ) {
  ...
  lava_foreach( ReadEffect* read , lhs.read_list_.GetForwardIterator() ) {
    effect_phi->AddEffect(read);
  }
  lava_foreach( ReadEffect* read , rhs.read_list_.GetForwardIterator() ) {
