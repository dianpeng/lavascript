...
void GraphBuilder::Environment::EnterFunctionScope( Graph* graph , const FuncInfo& func ) {
  ...
  upvalue_.resize( upvalue_.size() + func.prototype->upvalue_size() );
  // initialize the upvalue
  {
    const std::size_t usz = func.prototype->upvalue_size();
...
void GraphBuilder::Environment::ExitFunctionScope( Graph* graph , const FuncInfo& func ) {
  ...
  upvalue_.resize( func.uv_base );
}

void GraphBuilder::Environment::UpdateEffect( GraphBuilder* gb , MemStore* effect ) {
  ...
  // We don't need to actively update global variable and upvalue, since they
  // are generated by special bytecode and we just need to add the effect at
  // that moment when special instruction is met
}
...
class GraphBuilder::FuncScope {
 public:
  FuncScope( GraphBuilder* , const Handle<Prototype>& , ControlFlow* , std::uint32_t , std::uint32_t );
  ~FuncScope() {
    gb_->env()->ExitFunctionScope(gb_->graph_,gb_->func_info_.back());
    gb_->func_info_.pop_back();
  }
...
void GraphBuilder::NewGSet( std::uint8_t a1 , std::uint8_t a2 ,
                                              ...
                                              bool sso ) {
  auto info = NewIRInfo(loc);
  auto gset = GSet::New(graph_, (sso ? NewSSO(a1,info) : NewString(a1,info)) ,StackGet(a2) ,
                                                                              info,
                                                                              region());
...
void GraphBuilder::InsertPhi( Environment* lhs , Environment* rhs , ControlFlow* region , IRInfo* info ) {
  ...
  GeneratePhi(upval(),*lhs->upvalue(),*rhs->upvalue(),func_info().uv_base,region,info);
...
void GraphBuilder::GenerateLoopPhi( const BytecodeLocation& pc ) {
  auto ir_info = NewIRInfo(pc);
  // 1. patch all the loop bounded variable
  {
  ...
  // 2. patch all the loop bounded upvalue
  {
    const std::size_t len = func_info().current_loop_header()->uv_phi.size();
    for( std::size_t i = 0 ; i < len ; ++i ) {
      if(func_info().current_loop_header()->uv_phi[i]) {
        Expr* old = UpValueGet(static_cast<std::uint32_t>(i));
        lava_debug(NORMAL,lava_verify(old););
        Phi* phi = Phi::New(graph_,region(),ir_info);
        phi->AddOperand(old);
        UpValueSet(static_cast<std::uint32_t>(i),phi);
        func_info().current_loop().AddUVPhi(static_cast<std::uint8_t>(i),phi);
      }
    }
  }
}
...
void GraphBuilder::PatchLoopPhi() {
  // 1. patch all the loop bounded phi
  {
  ...
  // 2. patch all the loop bounded upvalue phi
  {
    for( auto & e: func_info().current_loop().uvphi_list ) {
      Phi*  phi = e.phi;
      if(phi->IsUsed()) {
        Expr* node = UpValueGet(e.idx);
        lava_debug(NORMAL,lava_verify(phi != node););
        phi->AddOperand(node);
      } else {
        Phi::RemovePhiFromRegion(phi);
    ...
    func_info().current_loop().uvphi_list.clear();
  }
}
