...
template< typename T > class TaggedPtr {
  static constexpr std::size_t CacluateBits( std::size_t size ) { return (size == 4 ? 2 : 3); }
 public:
 ...
 public:
  explicit TaggedPtr( T* p = NULL )       { CheckPtr(p); ptr_ = p; }
  TaggedPtr( T* p , std::uint32_t state ) { reset(p,state); }
  // get the usable address/pointer
  T* ptr() const                          { return reinterpret_cast<T*>(raw_ & kPtrMask); }
  ...
  void reset( T* p ) { CheckPtr(p); ptr_ = p; }
