...
class Writer:
    ...
    def put_header(self):
        self.put_line("// ========================================================================")
        self.put_line("//   This File Is Generated by hir-preprocessor.py ! Do Not Modify !")
        self.put_line("// ========================================================================")
        self.put_line()
...
class HIRModel:
    ...
    def _parse(self,src,start):
        ...
        sub_node.meta = meta
        sub_node.meta["class"] = sub_node.name
...
class CxxXMacroGenerator:
    ...
    def _visit_node(self,node):
        if self._predicate_node(node):
            try:
                temp = "__({0})".format(self.temp)
                temp = temp + " {dim}"
                self.writer.put_line(temp.format(dim="\\",**node.meta))
            except:
                _stderr("node {0}'s meta tag is not defined correctly".format(node.name))
    ...
    def generate(self):
        ...
        self.writer.put_line("#define {0}(__) \\".format(self.name))
...
def _xmacro(fn,model,name,temp,base,leaf):
    with open(fn,"w+") as f:
        def w(msg):
            f.write(msg)
        writer = Writer(w,"  ")
...
if __name__ == "__main__":
    ...
    elif arg.xmacro is not "":
        if arg.xmacro_temp == "" or arg.xmacro_name == "" or arg.xmacro_leaf is None:
            bitch_and_die()

        base = None
        if arg.xmacro_base != "":
            base = arg.xmacro_base

        _xmacro(arg.xmacro,model,arg.xmacro_name,arg.xmacro_temp,base,arg.xmacro_leaf)
    else:
        parser.print_help()
